...
(function($) {
  ...
  function __printComment($this, comment, position) {
    ...
    if (settings.author && ((settings.author.id == comment.author.id) || settings.author.anonymous))
      ...
      $("<span>").addClass("date").text(" - " + comment.creationDate).appendTo($("<p>").addClass("author").append($('<span>').text(comment.author.fullName)).appendTo(commentBox));
    //.userZoom(comment.author)
